...
export default function ProfileSettings() {
  ...
  const [nickname, setNickname] = useState("Togzhan");
  ...
  useEffect(() => {
    ...
    const legacyNick = savedProfile.nickname || savedProfile.fullName;
    setNickname(
      legacyNick || savedUser.nickname || savedUser.username || "Togzhan"
    );
  ...
  const handleSaveProfile = async () => {
    ...
    if (nick.length < 2) {
      toast.error(text.nicknameTooShort || "Nickname must be at least 2 characters.");
      return;
    }
    try {
      const u = await patchMe({ nickname: nick });
      localStorage.setItem("user", JSON.stringify(u));
      localStorage.setItem("userName", u.nickname || nick);
      const profileData = {
